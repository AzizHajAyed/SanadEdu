...
#Creating the pie chart for the Comfort Level in School Surroundings
column_data = df["2. How would you rate the quality of your relationships with your classmates?"]
tab = pd.to_numeric(column_data, errors='coerce').dropna().astype(int)
value_counts = tab.value_counts()
plt.figure(figsize=(8, 8))
plt.pie(value_counts, labels=value_counts.index, autopct='%1.1f%%', startangle=140)
plt.title('Quality Of Relationships With Classmates')
plt.axis('equal')
